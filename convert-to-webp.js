...
async function main() {
  ...
  results.forEach((r) => {
    ...
    console.log(`  PNG: ${r.originalSize}MB → WebP: ${r.webpSize}MB (小 ${r.savings}%)`);
  });
  
  const totalSavings = ((1 - totalWebP / totalOriginal) * 100).toFixed(1);
  
  console.log("═".repeat(70));
  console.log(`总计 PNG: ${totalOriginal.toFixed(2)}MB → WebP: ${totalWebP.toFixed(2)}MB`);
  console.log(`额外节省: ${(totalOriginal - totalWebP).toFixed(2)}MB (${totalSavings}%)`);
  console.log("\n✨ 完成！所有图片已转换为 WebP 格式。");
