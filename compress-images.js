...
async function main() {
  ...
  results.forEach((r) => {
    ...
    console.log(`${r.path}`);
    console.log(
      `  ${r.originalSize}MB → ${r.compressedSize}MB (节省 ${r.savings}%)`
    );
  });
  ...
  console.log("═".repeat(60));
  console.log(
    `总计: ${totalOriginal.toFixed(2)}MB → ${totalCompressed.toFixed(2)}MB`
  );
  console.log(
    `节省: ${(totalOriginal - totalCompressed).toFixed(2)}MB (${totalSavings}%)`
  );
  console.log("\n✨ 完成！所有图片已压缩。");
}
