...
async function optimizeImage(config) {
  try {
    ...
    console.log(`\n优化: ${config.input}`);
    console.log(`  当前 WebP: ${oldWebpSizeMB}MB → 目标 quality: ${config.quality}`);
    ...
    console.log(`✅ 新 WebP: ${newWebpSizeMB}MB${savings !== "N/A" ? ` (再节省 ${savings}%)` : ""}`);
...
async function main() {
  ...
  if (results.length > 0) {
    ...
    console.log(`再节省: ${(totalOld - totalNew).toFixed(2)}MB (${totalSavings}%)`);
  }
