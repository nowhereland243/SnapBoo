  ...
  try {
    ...
    if (ext === ".png") {
      ...
      if (newSize < originalSize * 0.95) {
        ...
        finalSize = newSize;
        optimized = true;
        console.log(`  ✓ PNG 压缩: ${originalSizeMB}MB → ${(newSize / 1024 / 1024).toFixed(2)}MB`);
      } else {
      ...
      if (newSize < originalSize * 0.95) {
        ...
        finalSize = newSize;
        optimized = true;
        console.log(`  ✓ JPG 压缩: ${originalSizeMB}MB → ${(newSize / 1024 / 1024).toFixed(2)}MB`);
      } else {
        fs.unlinkSync(fullPath + ".optimized");
...
async function main() {
  ...
  if (optimizedFiles.length > 0) {
    ...
    optimizedFiles.forEach((r) => {
      console.log(`  ${r.path}`);
      console.log(`    ${r.originalSize}MB → ${r.finalSize}MB (节省 ${r.savings}%)`);
    });
  }
  ...
  console.log(`  节省:     ${(totalOriginal - totalFinal).toFixed(2)}MB (${((1 - totalFinal / totalOriginal) * 100).toFixed(1)}%)`);
